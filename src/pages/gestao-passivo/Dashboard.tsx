...
// Dados simulados de evolução de provisão por carteira (0-24 meses)
const generateEvolutionData = () => {
  const data = [];
  for (let mes = 0; mes <= 24; mes++) {
    // Fórmulas aproximadas baseadas na BCB 352
    const c1 = Math.min(5.5 + (mes * 4.5), 100); // Cresce mais devagar
    const c2 = Math.min(30 + (mes * 3.4), 100); // Cresce médio
    const c3 = Math.min(45 + (mes * 2.6), 100); // Cresce rápido
    const c4 = Math.min(35 + (mes * 3.0), 100); // Cresce médio-rápido
    const c5 = Math.min(50 + (mes * 2.4), 100); // Cresce mais rápido
...
export default function Dashboard() {
  ...
  // Agrupar por marcos de provisionamento
  ...
  const proximasMudancas = useMemo(() => {
    return analisesFiltradas
      .map((analise) => {
        ...
        // Estimar dias para mudança (assumindo crescimento linear - simplificado)
